...
[CustomPropertyDrawer(typeof(ThunderEvent))]
public class ThunderEventDrawer : PropertyDrawer
{
    ...
    static public string[] GetNames(List<Entry> list, string selectedClass, string selectedMethod, bool includeParams, out int index, SerializedProperty methodProp = null)
    {
        index = 0;
        ...
        for (int i = 0; i < imax; entry = null)
        {
            ...
            string methodGroup = entry.name;
            string methodName = "";
            
            if (string.IsNullOrEmpty(methodGroup) == false && methodGroup.Contains(" ("))
                methodName = methodGroup.Remove(methodGroup.IndexOf(" ("));
            else
				methodName = methodGroup;
            
            methodGroup = ThunderEvent.GetFuncName(entry.target, methodGroup);

          
            if (includeParams)
                names [++i] = ThunderEvent.GetFuncName(entry.target, entry.name);
            else
                names [++i] = methodGroup;
            
            if (index == 0)
            {
                if (ThunderEvent.GetClassName(entry.target).ToLower().Equals(selectedClass.ToLower()) && selectedMethod.ToLower() == methodName.ToLower())
                {
                    index = i;
                }
                /*
                else if (string.Equals(methodGroup, selectedMethod))
                {
                    index = i;
                }
                */
    ...
    static public List<Entry> GetMethods(GameObject target, bool includeParams = false)
    {
        ...
        for (int i = 0, imax = comps.Length; i < imax; ++i, comp = null)
        {
			...
			methods = comp.GetType().GetMethods(ThunderEvent.MethodFlags);
		...
		methods = target.GetType().GetMethods(ThunderEvent.MethodFlags);
	...
	static private void AddProperties(List<Entry> list, UnityEngine.Object comp, bool read, bool write)
	{
        ...
        BindingFlags flags = ThunderEvent.FieldFlags;
